      ...
      <div className="grid grid-cols-3 gap-4 place-items-center p-4">
        {mehsul.length === 0 ? (
          ...
          mehsul.map((item, i) => (
            <div
              key={i}
              className="max-w-sm bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col justify-between"
            >
              {/* Şəkil Konteyneri */}
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  ...
                  <button onClick={()=>addbasket(item.id,item.title,item.price,item.image,item.category)} className="bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium px-5 py-3 rounded-xl transition-all active:scale-[0.98] shadow-sm hover:shadow-md">
                    Səbətə at
                  </button>
                </div>
              </div>
